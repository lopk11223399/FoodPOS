...
const amountBill = document.querySelector('.bill__amount')
const billShow = document.querySelector('.bill__icon')
const UserName = document.querySelector('.header__title--name')
const logout = document.querySelector('.sidebar__logout')
...
class UI {
    ...
    // render products in cart to bill
    addCartToBill() {
        cart = Storage.getCart()
        this.renderCartPayment(cart)
    }
    ...
    payment() {
        ...
        billShow.addEventListener('click', () => {
            this.showBill()
            this.addCartToBill()
        })
    }
